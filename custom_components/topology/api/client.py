...
class TopologyApiClientError(Exception):
    """Base exception to indicate a general API error."""


class TopologyApiClientCommunicationError(
    TopologyApiClientError,
):
    """Exception to indicate a communication error with the API."""


class TopologyApiClientAuthenticationError(
    TopologyApiClientError,
):
...
def _verify_response_or_raise(response: aiohttp.ClientResponse) -> None:
    ...
    Raises:
        TopologyApiClientAuthenticationError: For 401/403 errors.
        aiohttp.ClientResponseError: For other HTTP errors.

    """
    if response.status in (401, 403):
        msg = "Invalid credentials"
        raise TopologyApiClientAuthenticationError(
            msg,
        )
...
class TopologyApiClient:
    ...
    async def async_get_data(self) -> Any:
        ...
        Raises:
            TopologyApiClientAuthenticationError: If authentication fails.
            TopologyApiClientCommunicationError: If communication fails.
            TopologyApiClientError: For other API errors.
    ...
    async def _api_wrapper(
        ...
        try:
            ...
            raise TopologyApiClientCommunicationError(
                msg,
            ) from exception
