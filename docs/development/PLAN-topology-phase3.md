...
- `area_slug = slugify(area.name)` (`homeassistant.util.slugify`) captured at
  creation time. Two areas that slugify identically get HA's `_2` disambiguator
  via `async_get_available_entity_id` — `unique_id` keeps them distinct.
...
- `_area_triple(coordinator, area_id)` returns the three `TopologyAreaSensor`
  instances (one per dimension). They are added even for areas with no
...
| `test_derive_areas_projection`               | `derive_areas` marks `exists`/`orphaned` and carries type/env/trust (unknown enum → None).          | hass, store_payload_full, area_registry     |
