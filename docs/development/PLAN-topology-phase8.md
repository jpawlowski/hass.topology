...
Last updated 2026-07-25 · **Decisions D1–D17 proposed, awaiting maintainer ratification.** Nothing in this
...
**Phase 8 adds no entity, attribute, WebSocket command, enum, service, store field, or derivation.** The only
code touched is `const.py` (a documentation-anchor map), `repairs.py` (two `learn_more_url` values),
...
`script/check`, `script/hassfest`, `script/test --cov` (≥ 93 % gate, 95 % measured), `script/frontend-check`,
...
D17 declines a genuinely valuable feature to keep the release phase a release phase).
...
| G6  | Coverage enforced at 93 %, ADR target 95 %                                                                                                                     | `test-coverage`                  | `DECISIONS.md` + CI                              |
| G7  | Repair cards have no per-issue **documentation** anchor; `learn_more_url` is fully spent on panel deep-links (8/10), the other two point at the bare repo root | `docs-troubleshooting` follow-on | `const.py` + `repairs.py` + CONFIGURATION.md     |
...
Verified by inspecting the entity attribute contracts, the label-projection format, and the service schemas.
**No service in `services.yaml` declares `supports_response`**, so there is no "call a service and read the
answer" escape hatch either — a blueprint's entire view of Topology is: the two always-enabled entities'
attributes, the optionally-enabled per-area sensors, and the optionally-projected labels.
...
R1–R4 and R6 are **by design** — master §1a deliberately keeps per-connection detail off the entity registry to
bound churn, and EXAMPLES.md states the rule plainly. R5 and R7 are the two that cost real capability, and D17
records the one additive change that would fix all of R1–R6 at once without adding a single entity.
...
| **New entity, attribute, WS command, enum, service, store field, or derivation**       | —        | **None.** Phase 8 is a release phase. D17 declines the one tempting addition and records it as the top post-1.0 candidate.                           |
...
| V3  | Doc anchors for all ten issues (G7)    | `tests/test_repairs_doc_anchors.py`: `ISSUE_DOC_ANCHORS` covers every `ISSUE_*` id; every anchor's fragment matches a heading present in `CONFIGURATION.md`; the two non-remediable cards are raised with their anchor; the eight remediable ones still carry their `homeassistant://` deep-link; ids, severities, placeholders, and fixability are unchanged. | automated   |
| V4  | Coverage gate (G6)                     | `script/test --cov` ≥ 93 % gate in CI; 95 % measured. Reviewed per PR, not just at release.                                                                                                                                                                                                                                                                    | automated   |
...
| D17 | A response-returning read service **(⚠️ declined, with reasons)**       | **Not in Phase 8.** Record it as the **top post-1.0 candidate**: one `topology.query` action with `SupportsResponse.ONLY` returning the `read_hook` payload would make R1–R6 reachable from any automation, script, or blueprint via `response_variable` — cardinal `side`, `glazed`, `passage`/`barrier`, the graph, `distance`, `level_delta`, and every `health` list at once. | It is the single highest-leverage unlock for `docs-examples` and for the Alarmo anchor consumer (R5), and it adds **no** entity and **no** registry churn. But it is a new service plus `services.yaml`, translations, and tests — i.e. it reopens Phase 6 inside the phase whose job is to ship. The four blueprints work without it. **The maintainer may pull it forward as an explicit scope decision;** if so, it is Phase 6.1, not Phase 8, and blueprint #2's user-supplied area list becomes a live query. |

**Explicit contradictions to ratify:** **D2** (retires the Core-merge framing of ADR "Quality Target" and, with
...
anchors, realized without disturbing Phase-7 D9's deep-links). **D17** is the one decision that _declines_
something valuable, and it is written to be easy to overturn.
