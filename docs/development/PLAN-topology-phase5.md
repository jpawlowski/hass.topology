...
carries credentials, tokens, or location coordinates** — but the registry
**identifiers themselves are name-derived** and so must be treated as PII (see
the identifier note below).

**Registry identifiers are name-derived (verified, Appendix A.5).** In HA
2026.4.4 `area_id` and `floor_id` are produced by `generate_id_from_name(name)`
— a slug of the display name (`annas_bedroom`), not a random ULID — and
topology's `edge_id` is the `f"{area_a}::{area_b}"` join of two such ids. So a
raw `area_id`/`floor_id`/`edge_id` (and a name-derived `sensor_entity_id` such
as `binary_sensor.annas_door`) leaks the same personal detail as the display
name even after names are redacted (PR-review r3645833645). Redacting them to
`**REDACTED**`, however, would collapse every distinct id to one token and
**destroy the graph joins** the bundle exists to show. The freeze therefore
requires **stable pseudonymization**, not raw retention and not flat redaction.

Frozen redaction rules for the Phase-6 export:

| Field / source                                                                                    | Rule                                                                                                                     | Reason                                                                                                                                                 |
| ------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Registry identifiers — `area_id`, `floor_id`, and the `sensor_entity_id` object part              | **pseudonymize**: replace with a stable token under a per-bundle salt (e.g. `area_1`, `floor_0`, or a salted short hash) | Name-derived (Appendix A.5), so raw values leak PII; a **consistent** mapping keeps every occurrence of the same id equal, so the graph stays legible. |
| `edge_id` (`area_a::area_b`)                                                                      | **pseudonymize**: rebuild from the two endpoints' pseudonyms                                                             | Preserves the adjacency join (`area_1::area_2`) without exposing either name-derived endpoint.                                                         |
| Registry-derived **area / floor display names** (if the export denormalizes them for readability) | **redact** via `async_redact_data`                                                                                       | Human-chosen names carry personal detail ("Anna's room"); nothing in the bundle needs the literal name.                                                |
| `AreaAnnotation.type` (open catalog — arbitrary user string)                                      | **redact** the value                                                                                                     | The only free-text annotation field (e.g. a custom `type`); could encode a personal label.                                                             |
| `sensor_entity_id` **domain** (e.g. `binary_sensor.`), enums, booleans, levels, timestamps        | keep                                                                                                                     | The domain and enumerated/structural values are non-sensitive and carry the debug signal (perimeter wiring shape, graph structure).                    |
| Orphaned entries (`orphaned_at` set)                                                              | keep (included, with ids pseudonymized like everything else)                                                             | ADR "Registry-Driven State" wants orphans in diagnostics for debuggability.                                                                            |

The pseudonym map is built once per export from a fresh salt (so bundles are not
cross-correlatable) and applied uniformly to every identifier field, so the
adjacency graph, perimeter set, and trust distribution remain fully analyzable
while no name-derived string survives. `async_redact_data` still handles the
display names and the free-text `type`; pseudonymization is the pass that runs
**before** it on the id fields.
...
example already shipped in `unknown_enum_after_downgrade`), and **no identifiers
or display names** (D9) — so the repair layer introduces nothing new to redact
or pseudonymize.
...
| D12 | Diagnostics-redaction rules — freeze here or defer with export? | **Freeze the ruleset here** (§6, spec only) to satisfy the §10 gate; **implement `diagnostics.py` in Phase 6**. Registry identifiers (`area_id`/`floor_id`/`edge_id`/`sensor_entity_id`) are **pseudonymized** (stable per-bundle map), not kept raw and not flat-redacted; names + free-text `type` go through `async_redact_data`. `diagnostics.py` stays the `{}` stub in Phase 5. | §10 lists redaction "Before Phase 5", but the export it governs is Phase 6 (§8 `diagnostics` row). Freezing the rule set (not the code) reconciles both. **Pseudonymization** added per PR-review r3645833645: HA `area_id`/`floor_id` are name-derived (`generate_id_from_name`, Appendix A.5), so raw ids leak PII, yet flat redaction would destroy the graph joins the bundle exists to show. |
...
### A.5 Name-derived registry identifiers (diagnostics-redaction basis, §6)

- `homeassistant/helpers/area_registry.py`: `AreaRegistry._generate_id(name)` returns `self.areas.generate_id_from_name(name)` (line 359) — the `area_id` is a **slug of the display name**, not a random ULID. A home set up as "Anna's Bedroom" carries `area_id = "annas_bedroom"`.
- `homeassistant/helpers/floor_registry.py`: `FloorRegistry._generate_id(name)` → `self.floors.generate_id_from_name(name)` (lines 175–177, used at line 179) — `floor_id` is likewise name-derived.
- Consequence for §6: raw `area_id`/`floor_id`, the topology `edge_id = f"{area_a}::{area_b}"` join, and a name-derived `sensor_entity_id` all leak the same personal detail as the redacted display name, so the Phase-6 diagnostics export must **pseudonymize** them under a stable per-bundle map rather than keep them raw (D12; PR-review r3645833645). Verified against the `home-assistant/core` `2026.4.4` tag.
