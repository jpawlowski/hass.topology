...
- Included connections (D15): every **exterior** connection (faces outside by
  definition), plus every **interior edge** connection where exactly one
  endpoint area has `environment == outdoor` (an opening onto a modelled outdoor
  area, e.g. a terrace door). Orphaned entries excluded.
...
| D15 | `connections_facing_outdoor` membership                     | All exterior connections + interior edges with exactly one `environment == outdoor` endpoint.                                 | Serves the west-covers / ventilation blueprints.                                                                                          |
