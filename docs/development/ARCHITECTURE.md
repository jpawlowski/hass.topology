...
This document describes the technical architecture of the Topology custom component for Home Assistant.
...
│   ├── __init__.py          # Exports TopologyDataUpdateCoordinator
...
│   ├── __init__.py          # Exports TopologyEntity
...
**Key class:** `TopologyDataUpdateCoordinator` (exported from `coordinator/__init__.py`)
...
- `TopologyConfigFlowHandler` (main flow)
- `TopologyOptionsFlow` (options)
...
**Key class:** `TopologyEntity` (in `entity/base.py`)
...
2. `TopologyEntity` for common functionality
...
1. Create directory: `custom_components/topology/<platform>/`
2. Implement `__init__.py` with `async_setup_entry()`
3. Create entity classes inheriting from platform base + `TopologyEntity`
