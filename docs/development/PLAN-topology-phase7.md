...
a confirm-step (+ the one-time onboarding import opt-in) and dropping the
now-duplicated `occupancy_extent` / projection-toggle / threshold fields from the
flow — **touches Phase-2 frozen artifacts and is ratified separately** (D14; see
§7). Phase 7 does not remove any flow field; it only routes "Configure" to the
panel, which is safe and reversible on its own.
