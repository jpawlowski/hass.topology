...
new WS command is needed** (D7 — answer: no), **panel admin-gating + audience**
...
| `custom_components/topology/__init__.py`                                                                                                            | **extend** | `async_setup` registers the static asset path once (`async_register_static_paths`, §4.3). `async_setup_entry` registers the panel (`panel_custom.async_register_panel`, admin-gated, `config_panel_domain=DOMAIN` so "Configure" opens the panel, §4.4/D14); `entry.async_on_unload` removes it (`frontend.async_remove_panel`). (§2.1, D4/D8/D14) |
...
config_panel_domain=DOMAIN, config={...})` (Appendix A.1/A.2). Removal is wired
  with `entry.async_on_unload(lambda: frontend.async_remove_panel(hass,
PANEL_URL_PATH, warn_if_unknown=False))` so an unload/reload cleanly drops the
  sidebar entry (`single_config_entry: true` means there is never a second panel
  to keep).

**`config_panel_domain=DOMAIN`** (verified param, Appendix A.1/A.2) makes the
panel the integration's **configuration surface**: the "Configure" button on the
topology tile in Settings → Devices & Services opens the panel instead of an
options dialog. This is the additive, Phase-7-local half of the config-surface
consolidation (D14) — settings already live in the panel via `update_home_config`
(§2.6). The _complementary_ half — **slimming the Phase-2 config flow itself** to
a **confirm-only** step, dropping the now-duplicated `occupancy_extent` /
projection-toggle / threshold fields, and **moving the one-time import opt-in out
of the flow into a panel first-run action** (ratified refinement: the panel is
the more flexible UX surface for onboarding) — **touches Phase-2 frozen artifacts
and is ratified separately** (D14; see §7). Phase 7 does not remove any flow
field; it only routes "Configure" to the panel, which is safe and reversible on
its own.
...
### 2.8 Native look & feel (design principle)

The panel should feel like part of HA's own area/floor management, not a bolted-on
app. Two facts bound how close we can get:

- **We cannot inject into the core Areas & Zones dialogs.** The area/floor edit
  dialogs and the Settings → Areas & Zones page are `home-assistant/frontend`
  components with **no extension point** for a third-party field or tab. A custom
  integration cannot add controls _inside_ them.
- **So the panel is a native-styled surface that sits beside them**, not inside
  them. It MUST use HA's design system — `ha-*` web-components, HA CSS custom
  properties / theme variables, the same card/dialog/list idioms — so it reads as
  a first-party area tool. It joins the same `hass.areas` / `hass.floors` the user
  already knows (D7), and `config_panel_domain` (§2.1) puts its entry where users
  expect integration config.
- **Label projection is the one place topology data appears _in_ the core area
  UI:** the projected `topology:<dim>:<value>` labels (Phase-6 §2.6) show up in
  the core area editor as labels. That is projection output, not an editing hook,
  but it is the native cross-surface visibility the model affords — worth leaning
  on in the UX so the two surfaces feel connected.

This is a styling/UX obligation on the Phase-7 frontend, not a new capability; it
adds no backend contract.
...
2. **Weigh the alternative.** The **canonical HA way** to make a repair issue
   ...
   real remediation is the panel). A fix*flow that merely \_links out* to the
...
**Card-reuse boundary (D15).** `api/ws-client.ts`, `api/types.ts`, and the
read-only render path (`map/floor-map.ts` + `map/layout.ts`) MUST be written
free of any panel-only assumption (no dependence on `route`/`panel` props, no
write-command imports, no editor imports) so a **future read-only Lovelace map
card** (v2+/Phase 7.5, §7) can import them unchanged. The write layer
(`editors/*`, the write half of `ws-client`) stays panel-only. This is a
structural requirement on the Phase-7 code, not a Phase-7 deliverable — no card
is built here; the modules are merely kept card-clean so the card is later a thin
...
| **Read-only Lovelace map card** (household-member consumption surface) | v2+ / Phase 7.5     | **Out of Phase-7 delivery**, but Phase-7 frontend modules are kept **card-reusable** (D15, §4.2) so the card is later a thin consumer of `ws-client`+renderer, not a rewrite. This is the non-admin "view the house map" answer — a dashboard card, **not** a non-admin panel (D15). |
| **Config-flow slimming to a confirm-step** (drop duplicated settings)  | Phase 2 (re-freeze) | **Out — separately ratified (D14).** Phase 7 only wires `config_panel_domain=DOMAIN` (additive, §2.1). Removing `occupancy_extent`/toggle/threshold fields from the flow touches the Phase-2 frozen config-flow field set + reconfigure flow and is a distinct decision.             |
...
| D8  | Panel admin-gating + audience                                                              | **`require_admin=True`** — admin-only sidebar entry. The non-admin "view the house map" need is met by a **read-only Lovelace card** (D15), not by opening the panel to users.                                                                                                                                                                                                                                                                                                         | **Primary rationale (strengthened):** topology is registry configuration ("Areas config++"), and HA gates **all** registry config (Areas, Floors, Devices, Settings) to admins — its _inputs_ are admin-only, so a user-facing editor would be inconsistent with HA's own mental model. (`@require_admin` per write command remains the hard boundary; the panel flag is UI gating.) The role-based "read-only for users, edit for admins" variant (`require_admin=False` + branch on `hass.user.is_admin`) is **feasible** but rejected for v1: it adds UI for a surface users cannot act on; the card (D15) is the cleaner consumption path. Cheap to revisit later.                                                                  |
...
| D14 | Config surface: panel vs. config/options flow **(⚠️ touches Phase 2 — ratify separately)** | **Two-part.** _Phase-7-local (safe, additive):_ wire `config_panel_domain=DOMAIN` so "Configure" opens the panel (§2.1). _Separate Phase-2 re-freeze:_ slim the config flow to a **confirm-only** step, **drop the duplicated** `occupancy_extent` / projection-toggle / `unannotated_repair_threshold` fields (already in the panel via `update_home_config`), and **move the one-time import opt-in into a panel first-run action** (ratified: panel = more flexible onboarding UX). | A config flow **must exist** (HA creates the entry only via a flow; Bronze `config-flow`/`test-before-configure` require it) — the lever is only how thin. Today the flow duplicates settings the panel already owns; the store is the source of truth and `entry.data` a cache. Slimming is more HA-idiomatic when a custom editing panel exists (ADR "Editing Surface"), but it edits the **Phase-2 frozen config-flow field set + reconfigure flow**, so it is a **distinct decision with a Phase-2 deprecation/migration window** — _not_ implemented in Phase 7. Phase 7 ships only `config_panel_domain` (reversible on its own). Ratify the direction; sequence the field removal into a Phase-2 follow-up.                      |
| D15 | Non-admin consumption surface (read-only map)                                              | **A read-only Lovelace map card** is the household-member "see the house" answer — **v2+/Phase 7.5, not built in Phase 7**. Phase-7 frontend modules are kept **card-reusable** (§4.2): `ws-client`/types + the read-only renderer carry no panel-only assumption.                                                                                                                                                                                                                     | Fully feasible non-core (custom cards are the canonical HACS frontend extension; no Core merge). Splits config (admin panel) from consumption (card for everyone) the HA-idiomatic way and keeps D8 admin-only clean. The only Phase-7 obligation is the module-boundary discipline so the later card is a thin consumer, not a rewrite.                                                                                                                                                                                                                                                                                                                                                                                                |
...
realized as `learn_more_url` values without touching frozen ids). **D14** is the
one decision that reaches **beyond Phase 7**: only its `config_panel_domain` half
is Phase-7 work; slimming the flow edits the **Phase-2 frozen** config-flow field
set + reconfigure flow and must be ratified and sequenced as a separate Phase-2
follow-up (with a deprecation/migration window), not folded into Phase-7
implementation. **D15** adds no Phase-7 deliverable beyond a module-boundary
...
- `panel_custom`'s manifest declares `"dependencies": ["frontend"]`; `frontend`
  ...
  order.
- `config_panel_domain` (verified param on both A.1 and A.2) is passed through to
  the built-in-panel config; it marks the panel as the **configuration surface**
  for the named domain, so the integration tile's "Configure" action opens the
  panel. Setting it to `DOMAIN` is the Phase-7-local mechanism behind D14. (What
  its `learn_more_url`-style in-app-vs-external rendering looks like is a frontend
  concern, but "Configure" opening the registered config panel is the documented
