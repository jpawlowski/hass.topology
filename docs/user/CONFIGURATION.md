...
This document describes all configuration options and settings available in the Topology custom integration.
...
```yaml
service: topology.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: topology.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
